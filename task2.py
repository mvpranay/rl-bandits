...
class PoissonDoorsEnv:
    ...
    def step(self, arm: int) -> Tuple[float, bool, Dict]:
        reward = float(self.rng.poisson(float(self.mus[arm])))
...
class StudentPolicy(Policy):
    ...
    def __init__(self, K: int, rng: Optional[np.random.Generator] = None):
        ...
        self.explore = True
        self.curr_arm_index = 0
        self.best_arm = None

        # shuffle arms
        self.arm_order = self.rng.permutation(K)
    ...
    def select_arm(self, t: int) -> int:
        ...
        curr_arm = self.arm_order[self.curr_arm_index]
        
        # if didn't pull enough times
        if self.counts[curr_arm] < self.P:
            return curr_arm
        # done pulling for this arm
        elif self.counts[curr_arm] == self.P:
            self.empirical_mean[curr_arm] = self.sums[curr_arm] / self.P

            # if mean good enough, switch to exploit
            if self.empirical_mean[curr_arm] >= self.T:
                self.explore = False
                self.best_arm = curr_arm
                return self.best_arm
            
            # otherwise move on to next arm
            self.curr_arm_index += 1

            # if no more arms left, switch mode to exploit
            if self.curr_arm_index == self.K:
                self.explore = False
                self.best_arm = np.argmax(self.empirical_mean)
                return self.best_arm
        
        return self.arm_order[self.curr_arm_index]
